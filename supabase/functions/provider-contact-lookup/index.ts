...
//   1) POST-style GET /v3/businesses/matches  -> best business id for name+address
...
//
// Request:  { name: string, address?: string, city?: string, state?: string, country?: string }
// Response: { found: boolean, phone?, display_phone?, yelp_url?, match_name?, message? }
// Consumed by src/pages/ProviderDirectory.jsx (ProfessionalCard).
...
Deno.serve(async (req: Request) => {
  ...
  try {
    // 1) Business Match — find the most likely business for this name + location.
    const matchUrl = new URL(`${YELP}/businesses/matches`)
    matchUrl.searchParams.set('name', name)
    matchUrl.searchParams.set('address1', address)   // may be empty; Yelp allows it
    ...
    // 2) Business Details — reliably get phone + Yelp page url.
