 ...
 * and applies prefill logic.
 *
 * Returns { questions, answers, loading }
 * where answers is a map: { [question_key]: SubmissionAnswer }
 */
import { useState, useEffect } from "react";
import { base44 } from "@/api/base44Client";
import { buildPrefillMap } from "./usePrefill";

// Fallback question bank for Weston permits when DB has no questions yet
const WESTON_QUESTION_BANK = {
  "Roofing — Full Replacement": [
    { section: "Property Information", question_key: "property_address", question_text: "What is the property address?", input_type: "address", is_required: true, display_order: 1, prefill_from: "property.full_address", target_system_field: "Property Address", target_system_section: "Property", help_text: "Enter the full street address of the property." },
    ...
    { section: "Project Details", question_key: "job_value", question_text: "Total estimated job value (labor + materials)?", input_type: "number", is_required: true, display_order: 5, prefill_from: "project.estimated_cost", target_system_field: "Job Value ($)", target_system_section: "Job Details", help_text: "Include all labor and materials. Required to determine permit fee." },
    ...
    { section: "Project Details", question_key: "noa_number", question_text: "Florida Product Approval (NOA) Number for new roofing material?", input_type: "text", is_required: true, display_order: 8, target_system_field: "NOA Number", target_system_section: "Job Details", help_text: "Find your product NOA at floridabuilding.org. Required for all roofing in HVHZ." },
    ...
    { section: "Contractor Information", question_key: "contractor_license", question_text: "Contractor State License Number", input_type: "text", is_required: true, display_order: 10, prefill_from: "contractor.license_number", target_system_field: "License Number", target_system_section: "Contractor", help_text: "Must be a valid Florida state license (e.g. CCC123456)." },
    ...
    { section: "Notices", question_key: "noc_required", question_text: "Is a Notice of Commencement (NOC) required?", input_type: "boolean", is_required: false, display_order: 12, target_system_field: "NOC Required", target_system_section: "Notices", help_text: "NOC is required for jobs over $2,500 in Weston (A/C: $15,000). Must be recorded at Broward County Recorder before work begins." },
  ],
  "HVAC / A/C Replacement": [
    ...
    { section: "Property Information", question_key: "folio_number", question_text: "BCPA Folio Number", input_type: "text", is_required: true, display_order: 2, prefill_from: "property.FOLIO_NUMBER", target_system_field: "Parcel / Folio Number", target_system_section: "Property" },
    { section: "Property Information", question_key: "owner_name", question_text: "Property Owner's Full Name", input_type: "text", is_required: true, display_order: 3, prefill_from: "property.NAME_LINE_1", target_system_field: "Owner Name", target_system_section: "Property" },
    { section: "Project Details", question_key: "job_value", question_text: "Total estimated job value ($)?", input_type: "number", is_required: true, display_order: 4, prefill_from: "project.estimated_cost", target_system_field: "Job Value ($)", target_system_section: "Job Details", help_text: "NOC required if job value exceeds $15,000 for HVAC in Weston." },
  ...
  "Window Replacement": [
    ...
    { section: "Project Details", question_key: "noa_number", question_text: "Impact Window Florida Product Approval (NOA) Number", input_type: "text", is_required: true, display_order: 6, target_system_field: "NOA Number", target_system_section: "Job Details", help_text: "All window replacements in Broward County (HVHZ) must use impact-rated windows with a valid Florida NOA." },
...
// Generates default questions for permit types not in the detailed bank
function getDefaultQuestions(permitTypeName) {
  return [
    { section: "Property Information", question_key: "property_address", question_text: "Property Address", input_type: "address", is_required: true, display_order: 1, prefill_from: "property.full_address", target_system_field: "Property Address", target_system_section: "Property" },
    { section: "Property Information", question_key: "folio_number", question_text: "BCPA Folio Number", input_type: "text", is_required: true, display_order: 2, prefill_from: "property.FOLIO_NUMBER", target_system_field: "Parcel / Folio Number", target_system_section: "Property" },
    { section: "Property Information", question_key: "owner_name", question_text: "Property Owner's Full Name", input_type: "text", is_required: true, display_order: 3, prefill_from: "property.NAME_LINE_1", target_system_field: "Owner Name", target_system_section: "Property" },
    { section: "Project Details", question_key: "job_value", question_text: "Total estimated job value (labor + materials)?", input_type: "number", is_required: true, display_order: 4, prefill_from: "project.estimated_cost", target_system_field: "Job Value ($)", target_system_section: "Job Details", help_text: "Include all labor and materials." },
...
export function useGuideQuestions(guide, currentUser) {
  const [questions, setQuestions] = useState([]);
  const [answers, setAnswers] = useState({}); // { [question_key]: SubmissionAnswer }
  ...
  useEffect(() => {
    ...
    load();
  }, [guide?.id, currentUser?.email]);

  const load = async () => {
    setLoading(true);

    // 1. Load questions from DB, fall back to built-in bank
    let dbQuestions = [];
    try {
    ...
    let qs = dbQuestions.length > 0
      ? dbQuestions
      : (WESTON_QUESTION_BANK[guide.permit_type_name] || getDefaultQuestions(guide.permit_type_name));
    ...
    // 2. Build prefill map
    const pmap = await buildPrefillMap(guide, currentUser);
    setPrefillMap(pmap);
    ...
    // 4. For unanswered questions with prefill_from, create a pending prefill entry
    qs.forEach(q => {
