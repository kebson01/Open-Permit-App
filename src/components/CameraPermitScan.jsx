          ...
          {pros === null ? (
            <>
              <p className="mt-1 text-sm text-gray-600">
                Want to see licensed {opt.contractor_category.toLowerCase()} contractors near you?
              </p>
              <button
              ...
              >
                {proLoading ? "Loading…" : "See licensed contractors"}
              </button>
            </>
          ) : pros.length > 0 ? (
            pros.map((c, i) => <ContractorCard key={i} c={c} />)
          ) : (
 ...
 *
 * The DBPR file has no phone number, so the number is matched on demand against
 * a business directory and never stored. The licence number is the part that
 * matters most — it is what lets someone confirm the person quoting them is
 * actually licensed and current — so it is always shown with a route to DBPR's
 * own verification search.
 */
  ...
  return (
    <div className="mt-3 border-b border-gray-100 pb-3">
      ...
      <p className="mt-0.5 text-xs text-gray-500">
        {[c.license_type, c.city, c.expires && `licence current to ${c.expires}`].filter(Boolean).join(" · ")}
      </p>
      ...
      <div className="mt-2 flex flex-wrap items-center gap-2">
        ...
        {phone === null ? (
          ...
          >
            {phoneLoading ? "Looking…" : "Find phone"}
          </button>
        ) : phone.found ? (
          <a
            href={`tel:${phone.phone || phone.display_phone}`}
            className="inline-flex items-center gap-1 rounded-lg bg-blue-600 px-2.5 py-1 text-[11px] font-semibold text-white no-underline"
          >
            📞 {phone.display_phone || phone.phone}
          </a>
        ) : (
